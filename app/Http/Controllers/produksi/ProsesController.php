...
use App\Http\Controllers\produksi\ProsesRule_Nama;
...
class ProsesController extends BaseController
{
    public function index(Request $request)
    {
        ...
        if (!empty($filterNama)) {
            $query->where('tb_trs_prc.nama', 'like', '%' . $filterNama . '%');
        }

        if ($filterStatus !== null && $filterStatus !== '') {
            $query->where('tb_trs_prc.status', $filterStatus);
        }
        ...
        if (isset($columns[$orderColumnIndex])) {
            ...
            if (in_array($orderColumnName, ['nama', 'status'])) {
                $field = match ($orderColumnName) {
                    'nama' => 'tb_trs_prc.nama',
                    'status' => 'tb_trs_prc.status',
                    default => 'tb_trs_prc.nama'
                };

                $query->orderBy($field, $orderDir);
            }
        } else {
            $query->orderBy('tb_trs_prc.nama');
        }
    ...
    public function add(Request $request)
    {
        ...
        // cek error
        $errList = array(
            'nama' => ['required', new ProsesRule_Nama($id, $nama)],
        );
